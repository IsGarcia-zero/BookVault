...
router.use(express.json()); // Middleware para procesar solicitudes con cuerpo JSON
router.use(express.urlencoded({ extended: true })); // Middleware para procesar solicitudes con datos en formato URL-encoded

router.post('/lector', (req, res) => {
  // Obtener los datos enviados desde el cliente
  const { nombreAutor, fecha, tituloLibro, usuarioId, libroId } = req.body;
  ...
  const libFavorito = {
    ...
    LIBRO_idLibro: libroId
  };

  connection.query('INSERT INTO favorito SET ?', libFavorito, (error, results) => {
    if (error) {
      console.error('Error al insertar en la base de datos:', error);
      res.status(500).json({ error: 'Error al insertar en la base de datos' });
    } else {
      console.log('Registro insertado correctamente en la base de datos');
      res.json({ message: 'Favorito marcado correctamente' });
    }
  });
});
